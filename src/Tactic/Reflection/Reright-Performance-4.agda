...
try-harder-ez : Bool × List Term → Term
try-harder-ez bts = -- try-harder' 0 (snd bts)
                    cps-length (snd bts) try-harder''

test-area : Set × Set × Set × Set
test-area = {!try fast!} , {!try slow!} , {!try-harder slow!} , {!try-harder-ez slow!}
