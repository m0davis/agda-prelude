...
record DeBruijn& {a} (A : Set a) {b} : Set (lsuc (a ⊔ b)) where
  field
    strengthenFrom& : (from n : Nat) → A → ∀ {B : Set b} → (Maybe A → B) → B
    weakenFrom&     : (from n : Nat) → A → ∀ {B : Set b} → (A → B) → B

  strengthen& : Nat → A → ∀ {B : Set b} → (Maybe A → B) → B
  strengthen& 0 x cc = cc (just x)
  strengthen& n x cc = strengthenFrom& 0 n x cc

  weaken& : Nat → A → ∀ {B : Set b} → (A → B) → B
  weaken& zero x cc = cc x
  weaken& n    x cc = weakenFrom& 0 n x cc
