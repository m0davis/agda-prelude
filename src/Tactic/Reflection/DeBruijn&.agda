...
record DeBruijn& {a} (A : Set a) : Set (lsuc a) where
  field
    strengthenFrom& : (from n : Nat) → A → Maybe A × Nat × A
    weakenFrom&     : (from n : Nat) → A → A × Nat × A

  strengthen& : Nat → A → Maybe A × Nat × A
  strengthen& 0 x = just x , 0 , x
  strengthen& n x = strengthenFrom& 0 n x

  weaken& : Nat → A → A × Nat × A
  weaken& zero x = x , zero , x
  weaken& n    x = weakenFrom& 0 n x
...
private
  Str : (V : Set → Set) → Set → Set
  Str V A = Nat → Nat → A → V (Maybe A)

  strVar : Str (_× Nat × Nat) Nat
  strVar lo n x = {!!}
...
--                         {A : Set a} {b} {{_ : DeBruijn& A {b} }} → DeBruijn& (F A) {b}
-- strengthenFrom& {{DeBruijnTraversable}} lo k x {B} cc = traverse {!!} {!!} {!!} -- traverse (strengthenFrom& lo k cc)
-- weakenFrom&     {{DeBruijnTraversable}} lo k x {B} cc = {!!} -- fmap     (weakenFrom& lo k cc)
-- -}
-- instance
