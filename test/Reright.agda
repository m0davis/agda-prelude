...
module Reright where
  ...
  simple-reright-test₁-helper : (A B : Set) (F : Set → Set) {w : Set} (A≡w : A ≡ w) → (solution : (F w) → A) → A
  simple-reright-test₁-helper a b f {._} refl fw = {!!}

  simple-reright-test₁ : (A B : Set) (F : Set → Set) → F A → A ≡ B → B → A
  simple-reright-test₁ A B F FA A≡B b = simple-reright-test₁-helper A B F A≡B {!!}
  ...
  simple-reright-test₂-helper : {a : Level} {A : Set a} {x y : A} {w : A} (x≡y₁ : x ≡ w) (_ : (x≡y₂ : x ≡ w) → w ≡ x) → w ≡ x
  simple-reright-test₂-helper {_} {_} {_} {_} {._} refl x = {!x!}

  simple-reright-test₂ : {a : Level} {A : Set a} {x y : A} (x≡y₁ : x ≡ y) (x≡y₂ : x ≡ y) → y ≡ x
  simple-reright-test₂ {y = y} x≡y₁ x≡y₂ = {!!}
...
{-
  module M₁ (A : Set) (a b : A) (a≡b : a ≡ b) where
  --           (A₁ : Set) {z : A₁} (z₁ z₂ : A₁) (z₃ z₄ : z ≡ z₂) (f : (A₂ : Set) (z₅ : z₂ ≡ z₂) → Set) (A₂ : Set) (z₅ : z ≡ z₂) → Set
  {-
  ...
  -}
  {-
    helper : (A₁ : Set) {z : A₁} (z₁ z₂ : A₁) (z₃ z₄ : z ≡ z₂) (f : (A₂ : Set) (z₅ : z₂ ≡ z₂) → Set) (A₂ : Set) (z₅ : z ≡ z₂) → Set
    helper _ {._} _ _ _ refl x = x

    test : Set → a ≡ b → Set
    test = helper A a b a≡b _ {!!}
           -- reright' 0 a≡b {!!}

    helper2 : (A₁ : Set) {z : A₁} (z₁ z₂ : A₁) (z₃ z₄ : z ≡ z₂) (f : (A₂ : Set) (z₅ : z₂ ≡ z₂) → A₁) (A₂ : Set) (z₅ : z ≡ z₂) → A₁
    helper2 _ {._} _ _ _ refl x = x

    test2 : Set → a ≡ b → A
    test2 = helper2 _ _ _ _ _ {!!}
            -- reright' 0 a≡b {!!}
  -}
